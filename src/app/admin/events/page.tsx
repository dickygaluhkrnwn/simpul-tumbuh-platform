    ...
    <div className="space-y-8 font-sans pb-20 selection:bg-primary-500 selection:text-white">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 glass-panel bg-white/70 p-6 md:p-8 rounded-[2.5rem] border border-slate-200/80 shadow-sm relative overflow-hidden">
        ...
        <div className="relative z-10">
          <h1 className="text-3xl md:text-4xl font-bold text-slate-900 font-uii mb-2 tracking-tight">Manajemen Event</h1>
          <p className="text-slate-600 font-medium">Kelola jadwal, kuota, dan detail kegiatan ekosistem.</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-sm whitespace-nowrap">
            <thead className="bg-slate-100/50 border-b border-slate-200/80">
              <tr>
                <th className="px-6 py-5 font-bold text-slate-700 uppercase tracking-wider text-xs">Nama Event</th>
                <th className="px-6 py-5 font-bold text-slate-700 uppercase tracking-wider text-xs">Jadwal & Lokasi</th>
                <th className="px-6 py-5 font-bold text-slate-700 uppercase tracking-wider text-xs text-center">Kuota</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {loading ? (
                ...
                filteredEvents.map((event) => {
                  ...
                  return (
                    <tr key={event.id} className="hover:bg-white transition-colors group">
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          <div className="w-14 h-14 rounded-xl bg-slate-100 overflow-hidden flex-shrink-0 border border-slate-200 shadow-sm">
                          ...
                          <div>
                            <p className="font-bold text-slate-900 text-base mb-1 max-w-[250px] truncate" title={event.title}>{event.title}</p>
                            <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-bold bg-slate-100 text-slate-600 uppercase tracking-widest border border-slate-200/60">
                              {event.category}
                            </span>
                          </div>
                        </div>
                      ...
                      <td className="px-6 py-4 text-center align-middle">
                        <div className="flex flex-col items-center justify-center">
                          <span className="font-bold text-slate-900 mb-1.5">{event.registered} <span className="text-slate-400 font-medium text-xs">/ {event.quota}</span></span>
                      ...
                      <td className="px-6 py-4 text-center">
                        <span className={`inline-flex px-3 py-1 rounded-full text-[11px] font-bold uppercase tracking-wider ${
                          event.status === 'open' ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' :
                          event.status === 'closed' ? 'bg-rose-50 text-rose-600 border border-rose-200' :
                      ...
                      <td className="px-6 py-4 text-right">
                        <div className="flex justify-end gap-2">
                          <Link href={`/admin/events/${event.id}/edit`}>
                            <button className="p-2 text-slate-400 hover:text-primary-600 hover:bg-primary-50 rounded-xl transition-all border border-transparent hover:border-primary-100" title="Edit Event">
                              <Edit size={18} />
                            </button>
                          </Link>
                          <button 
